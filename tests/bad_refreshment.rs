...
pub fn bad_refreshment1() {
    ...
    let (receiver_account, _receiver_balance) =
        testing::create_account_with_amount(&mut rng, 0);
    ...
    let fake_witness = CommitmentWitness::new(fake_balance.into(), proof_gen.balance_refresh_enc_blinding);
    ...
    proof_gen.balance_refreshed_same_proof = Some(single_property_prover(
    ...
    eprintln!("-- refreshment proof: {:?}", proof_gen.balance_refreshed_same_proof);
...
pub fn bad_refreshment2() {
    ...
    eprintln!("verify sender proof: {res:?}");
    assert_eq!(res, Err(Error::CiphertextRefreshmentFinalResponseVerificationError { check: 1 }));
}
